...
def train_model_func(learning_rate_rbm, learning_rate, batch_size, feature, label, path_out_png, path_out_txt,
    ...
    for i in range(train_deep, label.shape[0]):
        x_train_trandition = np.array(feature[0:train_deep])
        y_trian_trandition = np.array(label[0:train_deep])
...
def main(data, target, path_out_png, path_out_txt, learning_rate_rbm=0.01, learning_rate=0.01, batch_size=16,
         train_deep=10, step=100):
    feature = np.array([])
    ...
    train_model_func(learning_rate_rbm=learning_rate_rbm, learning_rate=learning_rate, batch_size=batch_size,
                     ...
                     train_deep=train_deep, step=step)
