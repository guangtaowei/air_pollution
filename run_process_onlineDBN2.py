...
def train_model_func(regressor_DBN, feature, label, path_out_png, path_out_txt, train_deep, regressor_DBN_prediction):
    ...
    for i in range(train_deep, label.shape[0]):
        ...
        x_range = range(i - train_deep + 1)
        plt.clf()
        plt.ylabel('value')
        plt.xlabel('hours')
        plt.title("DBN_online")
        plt.plot(x_range, prediction_trandition, marker='o', label="DBN_tradition")
...
def main(data, target, path_out_png, path_out_txt, learning_rate_rbm=0.001, learning_rate=0.001, batch_size=2,
         train_deep=1, step=100, tradition_train_deep=100):
    feature = np.array([])
    ...
    train_model_func(regressor_DBN=regressor_DBN, feature=feature, label=label, path_out_png=path_out_png,
                     path_out_txt=path_out_txt, train_deep=train_deep, regressor_DBN_tradition=regressor_DBN_tradition)
